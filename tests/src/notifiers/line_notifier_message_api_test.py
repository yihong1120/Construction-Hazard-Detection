...
class TestLineMessenger(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.messenger = LineMessenger(channel_access_token=self.channel_access_token, recipient_id=self.recipient_id)
    ...
    def test_init_without_channel_access_token(self):
        ...
        self.assertEqual(str(context.exception), 'LINE_CHANNEL_ACCESS_TOKEN not provided or in environment variables.')
    ...
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.upload_image_to_cloudinary')
    def test_push_message_with_image(self, mock_upload_image_to_cloudinary, mock_post):
        """
        Test sending a message with an image via LINE Messaging API.
        """
        mock_upload_image_to_cloudinary.return_value = ('http://mock_image_url', 'mock_public_id')
        mock_post.return_value.status_code = 200

        response_code = self.messenger.push_message(self.message, image_bytes=self.image_bytes)

        # Verify that the upload_image_to_cloudinary method was called
        mock_upload_image_to_cloudinary.assert_called_once_with(self.image_bytes)
        ...
        mock_post.assert_called_once_with('https://api.line.me/v2/bot/message/push', headers=self.headers, json=expected_data)
    ...
    def test_upload_image_to_cloudinary_success(self, mock_upload):
        ...
        """
        mock_upload.return_value = {'secure_url': 'http://mock_image_url', 'public_id': 'mock_public_id'}

        image_url, public_id = self.messenger.upload_image_to_cloudinary(self.image_bytes)

        # Verify that the Cloudinary uploader was called with the correct arguments
        mock_upload.assert_called_once_with(self.image_bytes, resource_type="image")
    ...
    def test_delete_image_from_cloudinary_success(self, mock_destroy):
        ...
        # Verify that Cloudinary's destroy method was called with the correct public_id
    ...
    def test_delete_image_from_cloudinary_exception(self, mock_destroy):
        ...
        with patch('builtins.print') as mock_print:
            self.messenger.delete_image_from_cloudinary('mock_public_id')
            mock_print.assert_called_once_with('Error deleting image from Cloudinary: Mocked exception')

    @patch('src.notifiers.line_notifier_message_api.LineMessenger.delete_image_from_cloudinary')
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.save_image_records')
    def test_delete_old_images(self, mock_save_image_records, mock_delete_image_from_cloudinary):
        """
        ...
        mock_delete_image_from_cloudinary.assert_called_once_with('old_image_id')
    ...
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.delete_old_images')
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.save_image_records')
    @patch('src.notifiers.line_notifier_message_api.datetime', wraps=datetime)
    def test_delete_old_images_with_interval(self, mock_datetime, mock_save_image_records, mock_delete_old_images):
        """
        ...
        self.messenger.image_records['last_checked'] = (now - timedelta(days=2)).isoformat()

        self.messenger.delete_old_images_with_interval()

        # Verify that the check time was updated
        self.assertEqual(self.messenger.image_records['last_checked'], now.isoformat())
    ...
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.push_message')
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.upload_image_to_cloudinary')
    @patch('src.notifiers.line_notifier_message_api.LineMessenger.__init__', return_value=None)
    def test_main(self, mock_init, mock_upload_image_to_cloudinary, mock_push_message):
        """
        ...
        with patch('builtins.print') as mock_print:
            main()
            mock_print.assert_any_call(f"Response code: 200")

        # Verify that the LineMessenger was initialized with the correct arguments
        mock_init.assert_called_once_with(channel_access_token="YOUR_LINE_CHANNEL", recipient_id="YOUR_RECIPIENT_ID")

        # Verify that the push_message method was called with the correct arguments
        mock_push_message.assert_called_once_with('Hello, LINE Messaging API!', image_bytes=frame_bytes)
    ...
    def test_load_image_records_failure(self, mock_file):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_once_with('Failed to load image records: Mocked exception')
    ...
    def test_save_image_records_failure(self, mock_file):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_once_with('Failed to save image records: Mocked exception')

    @patch('src.notifiers.line_notifier_message_api.LineMessenger.upload_image_to_cloudinary')
    def test_push_message_image_upload_failure(self, mock_upload_image_to_cloudinary):
        """
        ...
        with self.assertRaises(ValueError) as context:
            self.messenger.push_message(self.message, image_bytes=self.image_bytes)
        self.assertEqual(str(context.exception), "Failed to upload image to Cloudinary")
