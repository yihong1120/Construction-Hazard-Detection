...
class TestLineBot(unittest.TestCase):
    ...
    def test_callback_ok(self) -> None:
        """Test that the webhook endpoint returns 'OK' when provided with a
        valid signature and body payload."""
    ...
    def test_handle_text_message(self) -> None:
        ...
        self.assertEqual(
            reply_req.messages[0].text, '您发送的消息是: Hello, World!',
        )

    def test_handle_text_message_empty_early_return(self) -> None:
        """Empty or whitespace-only message should early return without
        reply."""
        event = SimpleNamespace(
            reply_token='dummy_token',
