...
class TestGetRedisPool(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_redis_pool(self, mock_from_url: AsyncMock) -> None:
        ...
        # Verify that the connect method was called via redis.from_url
...
class TestGetRedisPoolWS(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_redis_pool_ws(self, mock_from_url: AsyncMock) -> None:
        ...
        # Build a mock WebSocket with the RedisClient instance in app.state
        mock_websocket = MagicMock(spec=WebSocket)
        fake_app = MagicMock()
        fake_app.state.redis_client = mock_redis_client
        mock_websocket.app = fake_app

        # Call get_redis_pool_ws
        client = await get_redis_pool_ws(mock_websocket)
        mock_from_url.assert_awaited_once()
        self.assertEqual(client, mock_redis_instance)
    ...
    @patch('redis.asyncio.from_url', new_callable=AsyncMock)
    async def test_get_redis_pool_ws_raises_runtime_error(
        self,
        mock_from_url: AsyncMock,
    ) -> None:
        """
        ...
        with self.assertRaises(RuntimeError) as context:
            await get_redis_pool_ws(mock_websocket)
        self.assertEqual(
            str(context.exception),
            'Redis client is not connected.',
        )
