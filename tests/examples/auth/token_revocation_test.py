...
def test_remaining_lifetime_handles_expired_values(
    ...
    assert (
        token_revocation._remaining_lifetime(
            {'jti': 'expired', 'exp': 90},
        )
        == 0
    )
...
def test_revoke_and_query_access_tokens() -> None:
    ...
    async def run_case() -> None:
        ...
        try:
            assert (
                await token_revocation.revoke_access_token(
                    redis,
                    {'jti': 'live', 'exp': 130},
                )
                is True
            ...
            assert (
                await token_revocation.revoke_access_token_jtis(
                    redis,
            ...
            assert (
                await token_revocation.is_access_token_revoked(
                    redis,
                    {'jti': 'live'},
                )
                is True
