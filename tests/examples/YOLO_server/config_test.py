...
class TestSettings(unittest.TestCase):
    ...
    def test_settings_with_env_variables(self):
        # Remove _env_file parameter to let pydantic read from environment variables
        settings = Settings()
        self.assertEqual(settings.authjwt_secret_key, 'test_secret_key')
        self.assertEqual(
            settings.sqlalchemy_database_uri,
            'mysql+pymysql://test_user:test_password@localhost/test_db',
        )
    ...
    def test_settings_with_default_values(self):
        ...
        self.assertEqual(
            settings.sqlalchemy_database_uri,
            'mysql+pymysql://user:passcode@localhost/construction_hazard_detection',
        )
        self.assertFalse(settings.sqlalchemy_track_modifications)
