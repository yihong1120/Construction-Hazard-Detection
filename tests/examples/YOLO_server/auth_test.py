...
from examples.YOLO_server.auth import auth_router
from examples.YOLO_server.auth import UserLogin
...
class TestAuth(unittest.TestCase):
    def setUp(self):
        ...
        self.user_cache_patcher = patch(
            'examples.YOLO_server.auth.user_cache', {},
        )
    ...
    def test_create_token_successful_login(self, mock_get_db):
        ...
        response = client.post(
            '/token', json={'username': self.username, 'password': self.password},
        )
    ...
    def test_create_token_incorrect_password(self, mock_get_db):
        ...
        response = client.post(
            '/token', json={'username': self.username, 'password': 'wrongpassword'},
        )
