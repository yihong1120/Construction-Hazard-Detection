...
class TestStreamingRouterHlsHelpers(unittest.IsolatedAsyncioTestCase):
    def test_overlay_and_media_session_helpers_cover_invalid_inputs(self) -> None:
        ...
        self.assertIsNone(
            routers._media_hls_session_cookie(
                'hazard_site_cam', None,
            ),
        )
        self.assertIsNone(
            routers._media_hls_session_cookie(
                'hazard_site_cam', 'invalid value',
            ),
        )
    ...
    def test_hls_uri_and_playlist_rewriting_preserve_media_query_and_auth(self) -> None:
        ...
        self.assertIn(
            ...
            routers._rewrite_hls_uri(
                f'{media_path}/part.ts', media_path, auth_query,
            ),
        )
        self.assertIn(
            '/hazard/media/hazard_site_camera/absolute.ts?foo=bar&mt=opaque-token',
            routers._rewrite_hls_uri(
        ...
        self.assertIn(
            'URI="/hazard/media/hazard_site_camera/key.bin?mt=opaque-token"', rewritten,
        )
        self.assertIn(
            '/hazard/media/hazard_site_camera/segment.ts?mt=opaque-token', rewritten,
        )

    def test_media_path_and_session_payload_helpers_handle_bad_values(self) -> None:
        self.assertEqual(
    ...
    async def test_demand_and_media_session_indexes_tolerate_failures(self) -> None:
        base_path = routers.build_media_path('SiteA', 'Camera1')
        overlay_path = routers.build_annotated_media_path(base_path, 'en')
        rds = AsyncMock()
        with patch.object(routers, '_touch_overlay_demand', AsyncMock()) as touch_overlay:
        ...
        touch_overlay.assert_awaited_once_with(
            rds, base_path, 'en', ttl_seconds=17,
        )
    ...
    async def test_refreshes_and_prunes_invalid_media_session_indexes(self) -> None:
        async def keys():
        ...
        await routers._refresh_playback_sessions_for_media_path(rds, 'hazard_site_cam')

    async def test_playback_session_errors_and_startup_input_validation(self) -> None:
        rds = AsyncMock()
        with patch.object(routers, '_load_playback_session', AsyncMock(return_value=None)):
            with self.assertRaises(HTTPException) as raised:
        ...
        with patch.object(routers, 'STREAM_PLAYBACK_STARTUP_WAIT_SECONDS', 0.0):
            await routers._wait_for_session_startup({})
        with patch.object(routers, 'STREAM_PLAYBACK_STARTUP_WAIT_SECONDS', 1.0):
            await routers._wait_for_session_startup({'created_at': 'not-a-date'})
    ...
    async def test_label_access_rejects_inactive_users(self) -> None:
        ...
        with patch.object(
            ...
            with self.assertRaises(HTTPException) as raised:
                await routers._authorise_label_access(credentials, AsyncMock(), 'SiteA')
        self.assertEqual(raised.exception.detail, 'inactive_user')

    async def test_internal_hls_playlist_maps_network_status_and_empty_errors(self) -> None:
        context = MagicMock()
        client = AsyncMock()
        ...
        with patch.object(routers.httpx, 'AsyncClient', return_value=context):
            with self.assertRaises(HTTPException) as raised:
                await routers._fetch_internal_hls_playlist('hazard_site_cam', media_query='')
        ...
        with patch.object(routers.httpx, 'AsyncClient', return_value=context):
            with self.assertRaises(HTTPException) as raised:
                await routers._fetch_internal_hls_playlist('hazard_site_cam', media_query='quality=low')
    ...
    async def test_session_scan_startup_and_playlist_input_guards(self) -> None:
        """Playback helpers fail closed when sessions or media auth are invalid."""
        ...
        rds = SimpleNamespace(
            ...
            get=AsyncMock(
                return_value=(
                    b'{"profile":"overlay","base_media_path":"hazard_site_cam",'
                    b'"language":"en"}'
                ),
            ),
        )
        ...
        with (
            patch.object(
                routers, 'STREAM_PLAYBACK_STARTUP_WAIT_SECONDS', 30.0,
            ),
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(HTTPException, 'session_not_found'):
                await routers.stream_playback_session_playlist('missing', request, rds)
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(HTTPException, 'missing_media_token'):
                await routers.stream_playback_session_playlist('present', request, rds)

    async def test_playback_batch_and_endpoint_auth_guards(self) -> None:
        """Batch expansion and playback endpoints reject missing identity safely."""
        ...
        with self.assertRaisesRegex(HTTPException, 'stream_batch_limit_exceeded'):
        ...
        with self.assertRaisesRegex(HTTPException, 'Invalid token'):
            await routers.release_stream_playback(request, credentials, AsyncMock())
        with self.assertRaisesRegex(HTTPException, 'Invalid token'):
    ...
    async def test_playback_release_playlist_and_listing_edge_paths(self) -> None:
        """Release cleanup and listing failures preserve their API contracts."""
        credentials = SimpleNamespace(subject={'username': 'alice'})
        rds = AsyncMock()
        with patch.object(
            routers,
        ...
        with (
            ...
            with self.assertRaisesRegex(HTTPException, 'invalid_media_playlist'):
                await routers.stream_playback_session_playlist('valid', request, rds)
        ...
        with (
            ...
            patch.object(
                routers, 'normalise_overlay_mode',
                return_value='backend',
            ),
            patch.object(
                routers, 'normalise_label_language',
                return_value='xx',
            ),
            patch.object(
                routers, '_allowed_overlay_languages',
                return_value={'en'},
            ),
        ):
    ...
    async def test_backend_overlay_metadata_builds_demand_and_ready_payload(self) -> None:
        """Backend overlays create SSE demand metadata for the requested language."""
        async def events():
            yield b'data: {}\n\n'
    ...
    async def test_remaining_playback_session_paths_fail_closed_or_refresh(self) -> None:
        """Session scans skip stale rows and valid updates remove old indexes."""
        base_path = routers.build_media_path('SiteA', 'Camera1')
        self.assertFalse(
            routers._opaque_media_session_allows_path(
                {
