...
class MetadataFanoutTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_one_reader_broadcasts_latest_frame_to_all_subscribers(self) -> None:
        ...
        first = await fanout.subscribe(MagicMock(), 'metadata-key', fetcher=fetcher)
        second = await fanout.subscribe(MagicMock(), 'metadata-key', fetcher=fetcher)
        release.set()
...
class MetadataHandlerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sse_reads_from_subscription_and_closes_it(self) -> None:
        ...
        subscription.get = AsyncMock(return_value={'id': '1-0', 'has_warning': True})
        ...
        with patch(
            ...
            self.assertEqual(await anext(iterator), b'retry: 15000\n: connected\n\n')
    ...
    async def test_websocket_push_sends_subscription_frame(self) -> None:
        ...
        with (
            ...
            count = await metadata_push_loop(
                websocket, MagicMock(), 'metadata-key', '127.0.0.1', 'alice',
            )
