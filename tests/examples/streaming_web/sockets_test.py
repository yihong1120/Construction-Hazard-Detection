...
class TestSockets(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        """
        Sets up the test environment by initializing Socket.IO server
    ...
    async def test_connect(self) -> None:
        ...
        with patch.object(self.sio, 'emit', new_callable=AsyncMock) as mock_emit, \
                patch.object(self.sio, 'start_background_task') as mock_background_task:
    ...
    async def test_disconnect(self) -> None:
        ...
        with patch('builtins.print', new_callable=AsyncMock) as mock_print:
    ...
    async def test_update_images(self) -> None:
        ...
        """
        with patch.object(self.sio, 'sleep', new_callable=AsyncMock) as mock_sleep, \
                patch.object(self.sio, 'emit', new_callable=AsyncMock) as mock_emit:
            mock_sleep.side_effect = [None, None, Exception('Stop Loop')]

            self.redis_manager.get_labels.return_value = ['label1', 'label2']
            self.redis_manager.fetch_latest_frames.side_effect = lambda label: [
            ...
            # Verifying that emit is called with the correct data for each label
            mock_emit.assert_any_await(
                'update',
                {
                    'label': 'label1',
                    'images': ['image_data_label1_1', 'image_data_label1_2'],
                    'image_names': ['image_name_label1_1', 'image_name_label1_2'],
                },
            )
            mock_emit.assert_any_await(
                'update',
                {
                    'label': 'label2',
                    'images': ['image_data_label2_1', 'image_data_label2_2'],
                    'image_names': ['image_name_label2_1', 'image_name_label2_2'],
                },
            )
