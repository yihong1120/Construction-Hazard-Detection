...
class TestViolationRoutes(unittest.TestCase):
    ...
    def test_only_canonical_filter_and_analytics_routes_exist(self) -> None:
        """Legacy aliases must not remain registered after the breaking change.
        """
        self.assertIn('/violations/filter-options', self.paths)
        self.assertIn('/violations/analytics', self.paths)
        self.assertNotIn('/filter-options', self.paths)
