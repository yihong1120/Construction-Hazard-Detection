...
import argparse
from examples.YOLOv8_train.train import YOLOModelHandler, main
...
class TestYOLOModelHandler(unittest.TestCase):
    def setUp(self):
        ...
        """
        self.model_name = 'models/pt/best_yolov8x.pt'
        self.handler = YOLOModelHandler(self.model_name)

    @patch('examples.YOLOv8_train.train.YOLO')
    def test_load_model_with_yaml(self, mock_yolo):
        """
    ...
    def test_load_model_with_pt_and_device_selection(self, mock_mps_available, mock_cuda_available, mock_yolo):
        """
    ...
    def test_load_model_with_unsupported_format(self):
        """
        Test that loading a model with an unsupported format raises ValueError.
        """
        with self.assertRaises(ValueError) as context:
            handler = YOLOModelHandler('unsupported_format.txt')
        self.assertEqual(str(context.exception), "Unsupported model format. Use '.yaml' or '.pt'")
    ...
    def test_train_model(self, mock_yolo):
        ...
        handler = YOLOModelHandler(self.model_name)
        handler.train_model(data_config='tests/cv_dataset/data.yaml', epochs=10, optimizer='auto')
        mock_model.train.assert_called_with(data='tests/cv_dataset/data.yaml', epochs=10, batch=handler.batch_size, optimizer='auto')

    @patch('examples.YOLOv8_train.train.YOLO')
    def test_train_model_without_loading(self, mock_yolo):
        """
        ...
        self.assertEqual(str(context.exception), 'The model is not loaded properly.')

    @patch('examples.YOLOv8_train.train.YOLO')
    def test_validate_model_without_loading(self, mock_yolo):
        """
    ...
    @patch('examples.YOLOv8_train.train.YOLO')
    def test_predict_image_without_loading(self, mock_yolo):
        """
    ...
    @patch('examples.YOLOv8_train.train.YOLO')
    def test_export_model_without_loading(self, mock_yolo):
        """
    ...
    @patch('examples.YOLOv8_train.train.YOLO')
    def test_save_model_without_loading(self, mock_yolo):
        """
    ...
    @patch('examples.YOLOv8_train.train.YOLO')
    def test_cross_validate_model_without_loading(self, mock_yolo):
        """
    ...
    @patch('examples.YOLOv8_train.train.YOLO')
    def test_save_model(self, mock_yolo, mock_save):
        """
        ...
        handler = YOLOModelHandler(self.model_name)
        handler.save_model('path/to/save/model.pt')
        mock_save.assert_called_with(mock_model.state_dict(), 'path/to/save/model.pt')

    @patch('examples.YOLOv8_train.train.KFold.split')
    @patch('examples.YOLOv8_train.train.YOLO')
    def test_cross_validate_model(self, mock_yolo, mock_kfold_split):
        """
        ...
        handler = YOLOModelHandler(self.model_name)
        handler.cross_validate_model(data_config='tests/cv_dataset/data.yaml', epochs=10, optimizer='auto', n_splits=2)
    ...
    def test_predict_image_sahi(self, mock_sahi_predict, mock_auto_detection_model):
        """
        Test predicting an image using SAHI.
        """
        mock_model = MagicMock()
        mock_auto_detection_model.return_value = mock_model
        mock_sahi_predict.return_value = MagicMock(object_prediction_list=[{"label": "test", "score": 0.9}])

        result = YOLOModelHandler.predict_image_sahi('models/pt/best_yolov8x.pt', 'path/to/image.jpg')
        mock_auto_detection_model.assert_called_with(model_type='yolov8', model_path='models/pt/best_yolov8x.pt', confidence_threshold=0.3)
        mock_sahi_predict.assert_called()
        self.assertIsNotNone(result)

    def test_predict_image_sahi_without_model_path(self):
        """
        Test that calling predict_image_sahi without a valid model path raises RuntimeError.
        """
        with self.assertRaises(RuntimeError) as context:
            YOLOModelHandler.predict_image_sahi(None, 'path/to/image.jpg')
        
        self.assertEqual(str(context.exception), 'The model is not loaded properly.')

    @patch('argparse.ArgumentParser.parse_args')
    @patch('examples.YOLOv8_train.train.YOLOModelHandler')
    def test_main(self, mock_handler_class, mock_parse_args):
        """
    ...
    @patch('argparse.ArgumentParser.parse_args')
    @patch('examples.YOLOv8_train.train.YOLOModelHandler')
    def test_main_with_cross_validate(self, mock_handler_class, mock_parse_args):
        """
        ...
        # Since cross-validation was used, we do not expect train_model or validate_model to be called separately
    ...
    @patch('argparse.ArgumentParser.parse_args')
    def test_main_exception_handling(self, mock_parse_args, mock_train_model):
        """
