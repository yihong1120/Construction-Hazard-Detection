...
from examples.user_management.app import app, db
...
class TestUserManagement(unittest.TestCase):
    ...
    def test_add_user_success(self, mock_add_user):
        response = self.client.post('/add_user', data={'username': 'testuser', 'password': 'password123'})
    ...
    def test_update_username_success(self, mock_update_username):
        response = self.client.put('/update_username', data={'old_username': 'oldname', 'new_username': 'newname'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_username_missing_data(self):
        response = self.client.put('/update_username', data={'old_username': 'oldname'})
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'Invalid input', response.data)
    ...
    def test_update_password_success(self, mock_update_password):
        response = self.client.put('/update_password', data={'username': 'testuser', 'new_password': 'newpassword123'})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Password updated successfully', response.data)
        mock_update_password.assert_called_once_with('testuser', 'newpassword123')
    ...
    def test_update_password_missing_data(self):
        response = self.client.put('/update_password', data={'username': 'testuser'})
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'Invalid input', response.data)
