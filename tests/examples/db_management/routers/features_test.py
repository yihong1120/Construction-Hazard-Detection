...
class TestFeatureRouter(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        This method initialises a mock database session, a super admin user,
        and example feature/group objects for use in each test.
        """
    ...
    async def test_endpoint_list_features_super_admin(
        ...
        result: list[Feature] = await features.endpoint_list_features(
            db=self.db_session,
            current_user=self.super_admin_user,
        )
    ...
    async def test_endpoint_update_group_feature_success(
        self,
        mock_update_group_features: AsyncMock,
    ) -> None:
        ...
        self.assertEqual(
            result,
            {'message': 'Group features updated successfully.'},
        )
    ...
    async def test_endpoint_update_group_feature_not_found(self) -> None:
        """Test updating group features for non-existent group raises 404
        error."""
        payload: GroupFeatureUpdate = GroupFeatureUpdate(
            group_id=999,
            feature_ids=[1, 2],
    ...
    async def test_endpoint_list_group_features(
        self,
        mock_list_group_features: AsyncMock,
    ) -> None:
