...
class TestEmailVerificationServiceCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    def test_url_and_profile_helpers(self) -> None:
        with patch.object(svc, 'settings', _settings(app_public_url='https://app.example/')):
            self.assertEqual(
    ...
    async def test_delete_existing_token_handles_bytes_and_absent_values(self) -> None:
    ...
    async def test_send_email_handles_configuration_and_http_failures(self) -> None:
        with patch.object(svc, 'settings', _settings(brevo_api_key='')):
            with self.assertRaises(HTTPException) as unconfigured:
                await svc._send_email_verification_email('user@example.com', 'user', 'https://url')
        ...
        with patch.object(svc, 'settings', _settings()):
            with patch.object(svc.httpx, 'AsyncClient', return_value=status_context):
        ...
        with patch.object(svc, 'settings', _settings()):
            with patch.object(svc.httpx, 'AsyncClient', return_value=network_context):
    ...
    async def test_signup_and_resend_email_paths(self) -> None:
        ...
        with patch.object(svc, 'settings', _settings()):
            with patch.object(svc, '_create_email_verification_token', new=AsyncMock(return_value='token')):
                with patch.object(svc, '_send_email_verification_email', new=AsyncMock()) as send:
                    result = await svc.send_signup_verification_email(user, self.redis)
        self.assertEqual(result['code'], 'verification_email_sent')
        ...
        with patch.object(svc, '_create_email_verification_token', new=AsyncMock(return_value='token')):
            with patch.object(
                ...
                with patch.object(svc, '_delete_token_by_raw_token', new=AsyncMock()) as delete:
                    with self.assertRaises(HTTPException):
                        await svc.send_signup_verification_email(user, self.redis)
        delete.assert_awaited_once_with('token', self.redis)

        with patch.object(svc, '_find_user_by_email', new=AsyncMock(return_value=user)):
            with patch.object(
                svc,
                ...
                result = await svc.resend_verification_email(' ALICE@example.com ', self.db, self.redis)
        self.assertEqual(result['code'], 'verification_email_sent')
        send_signup.assert_awaited_once_with(user, self.redis)

    async def test_verify_email_token_rejects_invalid_payload_and_missing_user(self) -> None:
        ...
        self.assertEqual(
            invalid_payload.exception.detail['code'], 'invalid_token',
        )
        ...
        with self.assertRaises(HTTPException) as missing_user:
            await svc.verify_email_token('token', self.db, self.redis)
        self.assertEqual(
            missing_user.exception.detail['code'], 'invalid_token',
        )

    async def test_verify_email_token_rejects_non_verifiable_statuses(self) -> None:
        for status, code in [
