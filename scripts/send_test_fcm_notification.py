...
def _load_env() -> None:
    """Load the repository `.env` file for local execution.

    The project environment provides ``python-dotenv`` as a required
    dependency for this utility.
    """
...
def _store_device_token(
    ...
    if args.platform is None:
        raise RuntimeError(
            'Use --platform when registering a test token.',
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help='Client platform required with --device-token.',
    )
...
def main() -> int:
    ...
    try:
        with httpx.Client(timeout=args.timeout) as client:
            ...
            store_result = _store_device_token(client, args, access_token)
